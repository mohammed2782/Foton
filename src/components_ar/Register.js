...
export class Register_ar extends  Component <{}> {
	constructor(props) {
		super(props);
	...
	async componentWillMount (){
		...
		if (this.state.custId !=null && this.state.custId !==undefined){
			const customerInfo = await api.getRegisterCustomerInfo(this.state.custId,'AR');
	...
	async _cancelReg(){
		Alert.alert(
		  'إلغاء التسجيل',
		  "هل تريد إلغاء التسجل الخاص بك؟",
		  [
			{text: 'إلغاء', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
			{text: 'نعم', onPress: () => this._confirmedCancel()},
		  ],
		  { cancelable: false }
		);
	}
	async _confirmedCancel(){
		...
		try{
			...
			await api.cancelRegisteration(zuidInPhone, this.state.custId, "AR");
	...
	async _registerCustomer(){
		...
		if (allGood){
			this.setState({isLoading:true});
			try{
				const result = await api.registerCustomer(zuid,custId, custName, custHP1, custEmail, custVin , this.state.playerid, 'AR'  );
				if (result.errorMessage!==null && result.errorMessage!==undefined)
					...
					if (result.custId !=null && result.custId !==undefined){
						...
						const customerInfo = await api.getRegisterCustomerInfo(result.custId,'AR');
						...
						await Alert.alert("التسجيل","لقد تم تسجيلك");
						this.setState({isRegistered:true});
						this.setState({updateInfo:false});
					}
				}
										...
										<ImageBackground source={require('../../img/inner/field.png')}
											...
											<TouchableHighlight onPress={()=> {this._registerCustomer();}} 
											 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
												<Text style={{color:"white",textAlign:"center",paddingBottom:7}}>سجل الأن</Text>
											</TouchableHighlight>
										</ImageBackground>
									</View>
									<View style={{flex:0.5}} />
							...
							<View style={{flexDirection:"row-reverse"}} >
								...
								<View style={{flexDirection : "row-reverse" , justifyContent:"center" , alignItems:"center", flex:1}}>
									<Button iconLeft onPress={()=> {this._updateRegInfo();}}
										...
										<Text style={{color:"white",fontSize:12,marginRight:5}}>تعديل</Text>
									</Button>
									<Button iconLeft onPress={()=> {this._cancelReg();}}
										...
										<Text style={{color:"#d1251a",fontSize:12,marginRight:10}}>إلغاء التسجيل</Text>
									</Button>
								</View>
