...
import api from '../api/api.js';
	export class HomeScreen_ar extends Component <{}> {
		constructor(props) {
			super(props);
			this.state = {
				active: false,
		...
		async _prepareData(){
			...
			try{
					...
					if (zuid ==null){
						...
						// set zuid when null
						zuid = await api.addAppUser(deviceID, maker, os , model, 'AR');
					...
					changeOil = await api.checkOilShouldChange(zuid , 'AR');
					...
					Alert.alert("خطأ في الأتصال بالشبكه, رجاء تحقق من إتصالك بالأنترنيت");
					console.log(error);
			}
		}
		...
		onOpened(openResult) {
			...
			if (routeScreen =='PROMO'){
				this.props.navigation.navigate("Promotions_ar");
			}else if(routeScreen =='NORMAL_NOTIFICATION'){
				this.props.navigation.navigate("ShortNews_ar");
			}else if(routeScreen =='SPECIALPROMO'){
				this.props.navigation.navigate("SpecialPromotions_ar");
			}
		...
		checkOil(){
			...
			if (this.state.shouldChangeOil){
					...
					onPress={() => this.props.navigation.navigate("OilPopUp_ar",{callBackMethod: this._methodToReloadOnOilPopUpClose})}>
		...
		<ImageBackground style={styles.backgroundImage}
			...
			<View style={{flexDirection : "column",justifyContent:"space-around" , flex:1 }}>
				...
				<View style={{flex:6 ,marginRight:20 }}>
					<ListItem button noBorder style = {styles.ListItemStyle} onPress={() => this.props.navigation.navigate("Products_ar")}>
					...
					<ListItem button noBorder style = {styles.ListItemStyle} onPress={() => this.props.navigation.navigate("Parts_ar")}>
						<View style = {styles.ViewTextAction}>
					...
					<ListItem button noBorder style = {styles.ListItemStyle} onPress={() => this.props.navigation.navigate("Centers_ar")}>
						<View style = {styles.ViewTextAction}>
					...
					<ListItem button noBorder style = {styles.ListItemStyle} onPress = {()=> this.props.navigation.navigate("ContactUs_ar")} >
						<View style = {styles.ViewTextAction}>
					...
					<ListItem button noBorder style = {styles.ListItemStyle} onPress = {()=> this.props.navigation.navigate("AboutUs_ar")}>
						<View style = {styles.ViewTextAction}>
