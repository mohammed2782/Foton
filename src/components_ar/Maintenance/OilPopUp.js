...
import {
  ...
  Input
	...
	export class OilPopUp_ar extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
		...
		async componentDidMount(){
			try{
				...
				try{
					const lastMileageVal = await api.getLastOilChangedMileage('AR',zuidInPhone);
		...
		async _saveOilChange(){
			...
			const newMileage = this.state.newMileage;
				...
				const result = await api.saveOilChangeMileage(zuid, newMileage , 'AR' );
				console.log(result);
						...
						<ImageBackground source={require('../../../img/mileage/button.png')}
							...
							<TouchableHighlight onPress={()=> {this._saveOilChange();}}
							 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
								<Text style={{fontSize:14,color:"#b4b3b5",textAlign:"center",padding:10,}}>حفظ</Text>
							</TouchableHighlight>
						</ImageBackground>
					</View>
					<View style={{flex:0.5}}/>
					...
					<View style={{flex:4,marginTop:0 , marginLeft:20 , marginRight:20 ,paddingLeft:5,paddingRight:5, 
						...
						<View style={{flex:1 , justifyContent:"center"}}>
							<Text style={{alignSelf:"center" ,fontSize:13 , color:"white" }}>
								أخر مره تم تغيير زيت المحرك: {this.state.lastMileage}
							</Text>
						</View>
						<View style={{flex:2}}>
							{this.showCheckForm()}
