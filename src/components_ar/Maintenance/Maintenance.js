...
export class Maintenance_ar extends Component <{}> {
	_isMounted = false;
	constructor (props){
		super(props);
		//alert("this is const");
	...
	async _prepareData(){
		...
		try{
				...
				const lastMileageVal = await api.getLastMileage('AR',zuidInPhone);
				const lastOilChangeVal = await api.getLastOilChangedMileage('AR',zuidInPhone);
				//alert("did mount");
				if (this._isMounted){
					...
					<ImageBackground source={require('../../../img/mileage/button.png')}
						...
						<TouchableHighlight onPress={()=> {this._saveNewMileage();}} 
						 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
							<Text style={{fontSize:14,color:"#b4b3b5",textAlign:"center",padding:10,}}>أدخل العداد الحالي</Text>
						</TouchableHighlight>
					</ImageBackground>
				</View>
				<View style={{flex:0.5}}/>
							...
							<View style={{flex:8}}>
								...
								<View style={{flex:2,flexDirection:"column", alignItems:"center",justifyContent : "flex-start"}}>
									...
									<View style={{flex:0.5,flexDirection:"row" , justifyContent:"center"}}>
										<Text style={{alignSelf:"center" ,fontSize:14 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>العداد الحالي</Text>
										<Text style={{alignSelf:"center",fontSize:11 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>{this.state.lastMileage}</Text>
									</View>
									<View style={{flex:0.5,flexDirection:"row" , justifyContent:"center"}}>
										<Text style={{alignSelf:"center" ,fontSize:14 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>أخر تغيير للزيت</Text>
									...
									<View style={{flex:0.3 , justifyContent:"center"}}>
										<Button iconLeft onPress={()=> {this._resetAll();}} transparent style={{borderColor: '#b4b3b5'}} textStyle={{color:'red'}}>
											...
											<Text style={{color: '#b4b3b5',fontSize:11}}>إعادة تهيئة العداد</Text>
										</Button>
									</View>
								</View>
							</View>
