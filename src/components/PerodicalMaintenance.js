...
import {
  ...
  ListItem,
	...
	export  class PerodicalMaintenance extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
		...
		async componentWillMount(){
			...
			if (!isThere){
				await alert("Your registeration had been cancelled");
			...
			const rangesListJSON = await api.fetchPerodicalRangesList('en', zuidInPhone);
		...
		return (
			<ImageBackground style={styles.backgroundImage} source={require('../../img/mileage/BG.png')} blurRadius={0} resizeMode = "cover">
				<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
					...
					<View style={{flex:1,marginTop:10,marginLeft:20,justifyContent : "center", alignItems:"flex-start"}}>
					...
					<View style={{flex:10,backgroundColor:"rgba(43, 75, 122,0.7)" , marginLeft:20 , marginRight:20}}>
						<FlatList contentContainerStyle={{}} data={this.state.rangesList} keyExtractor={item => item.rangeId.toString()}
							renderItem={({item}) =>(
							<LinearGradient style={{margin:15,marginTop:10,flex:1,flexDirection :"row"}} colors={["#090f17", "transparent"]} 
								start={{x:0, y:0}} end={{x:1, y:0}}locations={[0,0.9]}>
								<View style={{flex:1,flexDirection :"row",alignItems:"center",marginBottom:5,padding:10}}>
									...
									<View style={{flex:7}}>
										<Button transparent  onPress={() => 
											this.props.navigation.navigate("AdviceTable",{rangeId : item.rangeId, startRange:item.startRange, endRange:item.endRange})}
											style={{justifyContent:"flex-start",alignItems: "center", flex:1, margin:0}}>
											<Text style={styles.carTitle}>From {item.startRange} - {item.endRange} km</Text>
										</Button>
									</View>
								</View>
							</LinearGradient>
