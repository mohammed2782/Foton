...
export class Register extends  Component <{}> {
	...
	async componentWillMount (){
		...
		if (this.state.custId !=null && this.state.custId !==undefined){
			const customerInfo = await api.getRegisterCustomerInfo(this.state.custId,'EN');
	...
	async _confirmedCancel(){
		...
		try{
			...
			await api.cancelRegisteration(zuidInPhone, this.state.custId, "EN");
	...
	async _registerCustomer(){
		...
		if (allGood){
			this.setState({isLoading:true});
			try{
				const result = await api.registerCustomer(zuid,custId, custName, custHP1, custEmail, custVin , this.state.playerid, 'EN'  );
				if (result.errorMessage!==null && result.errorMessage!==undefined)
					...
					if (result.custId !=null && result.custId !==undefined){
						...
						const customerInfo = await api.getRegisterCustomerInfo(result.custId,'EN');
			...
			<KeyboardAwareScrollView keyboardShouldPersistTaps={"always"} >
				<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
					<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
						<View style={{flexDirection : "row" ,marginTop:10, flex:2,paddingLeft:0}}>
							<View style={{flex:0.5}}/>
										...
										<ImageBackground source={require('../../img/inner/field.png')}
											...
											<TouchableHighlight onPress={()=> {this._registerCustomer();}} 
											 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
												<Text style={{color:"white",textAlign:"center",paddingBottom:7}}>Ok</Text>
											</TouchableHighlight>
										</ImageBackground>
									</View>
									<View style={{flex:0.5}} />
				...
				<ImageBackground style={styles.backgroundImage}
					...
					<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
						...
						<View style={{flexDirection : "row" , justifyContent:"flex-start" , alignItems:"center", flex:1,
							...
							{this.state.custId ?<Text style={{fontFamily:"Franklin Gothic Book Regular" ,color:"white", fontSize:15, fontWeight:"bold" ,color:"white"}}>
								Registeration Info
							</Text>
							:
						...
						<View style={{flex:9 , marginLeft:30, marginRight:30,}}>
							...
							<View style={{flexDirection:"row"}} >
								...
								<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
									<Button iconLeft onPress={()=> {this._updateRegInfo();}}
										...
										<Text style={{color:"white",fontSize:12,marginRight:5}}>Update</Text>
									</Button>
									<Button iconLeft onPress={()=> {this._cancelReg();}}
										...
										<Text style={{color:"#d1251a",fontSize:12,marginRight:10}}>Cancel Registeration</Text>
									</Button>
								</View>
...
const styles = StyleSheet.create(
	{
		...
		labelStyle : {fontFamily:"Franklin Gothic Book Regular" , fontSize:13 ,color:"#e2e1d9" , paddingRight : 10 , marginBottom:10},
		progressBar: {
