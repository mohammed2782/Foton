...
export class ContactUs extends  Component <{}> {
	...
	async _contactSend(){
		...
		if (allGood){
			...
			const result = await api.sendContactUsMsg(this.state.zuid, custName, custHP1, custEmail, msg, 'EN'  );
			...
			if (result.errorMessage!==null && result.errorMessage!==undefined)
				...
				Alert.alert('Contact us','We have recieved your inquiry, we will contact you soon.\nThank you');
	...
	render() {
		...
		<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
				...
				<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
					<View style={{flexDirection : "row" ,height:"12%",marginTop:20,alignItems:"flex-start", paddingLeft:0}}>
						<View style={{flex:2, justifyContent : "flex-start", alignItems:"center"}}>
							<Button	transparent onPress={() => this.props.navigation.goBack()}>
								<Image source={require("../../img/inner/backwhite.png")}  style={{ flex:1,
									width: "100%",
									height: "100%",
									resizeMode: 'contain'}}/>
					...
					<View style={{flex:1,flexDirection : "row" , justifyContent:"flex-start" , alignItems:"center", flex:1,
									...
									<ImageBackground source={require('../../img/inner/send_button.png')}
										...
										<TouchableHighlight onPress={()=> {this._contactSend();}} 
										 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
											<Text style={{color:"white",textAlign:"center",paddingBottom:10}}>Send</Text>
										</TouchableHighlight>
									</ImageBackground>
								</View>
								<View style={{flex:0.5}} />
