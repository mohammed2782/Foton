...
import {
  ...
  Input
	...
	export class OilPopUp extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
		...
		async componentDidMount(){
			try{
				...
				try{
					const lastMileageVal = await api.getLastOilChangedMileage('EN',zuidInPhone);
		...
		async _saveOilChange(){
			...
			const newMileage = this.state.newMileage;
				...
				const result = await api.saveOilChangeMileage(zuid, newMileage , 'EN' );
				console.log(result);
			...
			<View style={{flex:1}}>
				<View style={{flex:1,flexDirection:"row"}}>
					...
					<View style={{flex:1}}>
						...
						placeholder = "Mileage when changed oil "
					...
					<View style={{flex:4,marginTop:0 , marginLeft:20 , marginRight:20 ,paddingLeft:5,paddingRight:5, 
						...
						<View style={{flex:1 , justifyContent:"center"}}>
							<Text style={{alignSelf:"center" ,fontSize:13 , color:"white" }}>
								Last changed oil at mileage: {this.state.lastMileage}
							</Text>
						</View>
						<View style={{flex:2}}>
							{this.showCheckForm()}
