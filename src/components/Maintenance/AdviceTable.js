...
export  class AdviceTable extends   Component <{}>{
	constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
		...
		async componentDidMount(){
			...
			const adviceListJSON = await api.fetchAdviceList(this.state.rangeId, 'EN');
    ...
    renderRow(data) {
		/*<Image
					...
					/>*/
		return (
			<View style={{ flexDirection: 'row' ,borderBottomWidth:0.5, borderRadius:3,borderColor: 'grey',}}>
				<View style={{flex:1, padding:10, borderRightWidth:0.5,borderColor: 'grey' }} >
        ...
        return (
			<ImageBackground style={styles.backgroundImage}
				...
				<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
					<View style={{flexDirection : "row" ,marginTop:20,alignItems:"flex-start", paddingLeft:0}}>
						<View style={{flex:0.5}}/>
					...
					<View style={{flex:1, justifyContent : "flex-start", alignItems:"flex-start" ,marginLeft:20 , marginTop:20}}>
						<Text style = {{color:'white', fontSize:15, fontWeight:"700", fontFamily:'Futura Md BT Bold'}}>
								 Recommendations {this.state.startRange} - {this.state.endRange} km
							</Text>
					</View>
