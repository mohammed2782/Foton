...
export class Maintenance_kr extends Component <{}> {
	_isMounted = false;
	constructor (props){
		super(props);
		//alert("this is const");
	...
	async _prepareData(){
		...
		try{
				...
				const lastMileageVal = await api.getLastMileage('KR',zuidInPhone);
				const lastOilChangeVal = await api.getLastOilChangedMileage('KR',zuidInPhone);
				//alert("did mount");
				if (this._isMounted){
	...
	async _resetAll(){
			Alert.alert(
			  'نويَبونةوةى ثيَوةر',
			  'نويَكردنةوةى خويَندنى ثيَوةرت دوةيَ؟',
			  [
				{text: 'نةخيَر', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
				{text: 'بةلَىَ', onPress: () => this._resetOptionHandle()},
			  ],
			  { cancelable: false }
			);
		}
					...
					<ImageBackground source={require('../../../img/mileage/button.png')}
						...
						<TouchableHighlight onPress={()=> {this._saveNewMileage();}} 
						 style={{flex:1,height:50,alignItems:"center",justifyContent:"center"}}>
							<Text style={{fontSize:14,color:"#b4b3b5",textAlign:"center",padding:10,}}>داخلَكردنى كيلؤمتر</Text>
						</TouchableHighlight>
					</ImageBackground>
				</View>
				<View style={{flex:0.5}}/>
		...
		<KeyboardAwareScrollView keyboardShouldPersistTaps={"always"}   contentContainerStyle={{flexGrow: 1}} >
			<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false} style={{ flex: 1 }}>
				<View style={{flexDirection:"column" ,flex:1, justifyContent:"flex-start",}}>
					<View style={{flexDirection : "row-reverse" ,flex:1,marginTop:20,alignItems:"flex-start", paddingLeft:0,}}>
						<View style={{flex:1}}/>
							...
							<View style={{flex:8}}>
								...
								<View style={{flex:2,flexDirection:"column", alignItems:"center",justifyContent : "flex-start"}}>
									...
									<View style={{flex:0.5,flexDirection:"row" , justifyContent:"center"}}>
										<Text style={{alignSelf:"center" ,fontSize:14 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>پێوه‌ری کیلۆمه‌تر</Text>
										<Text style={{alignSelf:"center",fontSize:11 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>{this.state.lastMileage}</Text>
									</View>
									<View style={{flex:0.5,flexDirection:"row" , justifyContent:"center"}}>
										<Text style={{alignSelf:"center" ,fontSize:14 , color:"#b4b3b5",paddingRight:5,paddingLeft:5 }}>کۆتاجاری گورینه‌ وه‌ی ڕۆن</Text>
									...
									<View style={{flex:0.3 , justifyContent:"center"}}>
										<Button iconLeft onPress={()=> {this._resetAll();}} transparent style={{borderColor: '#b4b3b5'}} textStyle={{color:'red'}}>
											...
											<Text style={{color: '#b4b3b5',fontSize:11}}>نویگردنه‌وه‌</Text>
										</Button>
									</View>
								</View>
							</View>
